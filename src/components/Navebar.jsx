...
import { NAV_LINKS } from '../constants/profile';
      ...
      <div className='max-w-full bg-background text-white shadow-2xl shadow-black/50 sticky top-0 z-50'>
        <motion.div className='w-[95%] md:w-[96%] lg:w-[95%] mx-auto flex items-center justify-between h-20'>
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.5 }}>
            <a href="#home" className='text-2xl font-bold font-mono'>Portfolio</a>
          </motion.div>
          ...
          {isOpen && (
            ...
            >
              ...
              <button
                ...
                className='flex items-center justify-center gap-1 mx-auto bg-gradient-to-br from-[#FA7F25] to-[#FD9426] text-white font-semibold px-5 py-2 rounded-2xl w-fit cursor-pointer'
              >
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.5 }} className='hidden md:flex gap-6 text-gray-400 text-base font-medium '>
            {NAV_LINKS.map(({ label, href }) => (
              <a key={href} className='hover:text-primary duration-300 cursor-pointer group relative' href={href}>{label}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.5 }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} className='hidden md:flex'>
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className='flex items-center justify-center gap-1 bg-gradient-to-br from-[#FA7F25] to-[#FD9426] text-white font-semibold px-5 py-2 rounded-2xl cursor-pointer hover:shadow-lg hover:shadow-primary/40 transition-shadow'
            >
              <MdOutlineFileDownload /> Resume
            </motion.button>
          </motion.div>
